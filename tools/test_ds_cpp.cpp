...
void build(int a, int b, int count, int step)
{
  ...
  for (i=0; i < a; ++i) {
	  ...
	  (map) = stbds_hmput_key_wrapper((map), sizeof *(map), (void*) &x, sizeof (map)->key, 0), (map)[((stbds_array_header *) ((map)-1) - 1)->temp].key = (i*step), (map)[((stbds_array_header *) ((map)-1) - 1)->temp].value = (i+1);
  }
  measure();
  churn_inserts = i;
  hmfree(map);
